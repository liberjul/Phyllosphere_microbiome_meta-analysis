...
srr_agg = pd.read_csv(sys.argv[1], sep = "\t", header = None)

for srr_acc in list(srr_agg[0]):
    print(srr_acc)
    ...
    if True:
        if "." in str(fwd_file) and "." in str(rev_file):
            ...
            if len(glob.glob(F'/mnt/home/liberjul/He_Lab/phyllosphere_meta-analysis/SRA_files/{target}_{region}_pe/JL_...._{srr_acc}_L001_R._001.fastq.gz')) > 0:
                print(F"Cutadapt-processed and qiime-named reads exist for {srr_acc}")
                ...
                print(F"PE reads don't exist for {srr_acc}, trying SE")
                inpath = F'/mnt/home/liberjul/He_Lab/phyllosphere_meta-analysis/SRA_files/uncut_seqs/{srr_acc}'
                ...
                if os.path.exists(inpath + ".fastq.gz") and not os.path.exists(F"{outpath}1_001.cutadapt.fastq.gz"):
                    ...
                    subprocess.run(cmd, shell = True)
                elif os.path.exists(F"{outpath}1_001.cutadapt.fastq.gz"):
                    print(F"Single end reads {srr_acc} output exists")
                elif os.path.exists(inpath + ".fastq.gz"):
                    print(F"Single end reads {srr_acc} input exists and output exists")
                else:
                    print(F"Single end reads {srr_acc} input doesn't exist")
